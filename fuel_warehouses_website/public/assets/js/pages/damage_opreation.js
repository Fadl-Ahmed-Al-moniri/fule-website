...
async function loadDamageOperations() {
    try {
        showLoader();
        const res = await getRequest(API_ENDPOINTS.Operations.damage, token);
        ...
        alert('Failed to load damage operations. Please check the console.');
        noOperationsMsg.classList.remove('hidden');
    } finally {
        hideLoader();
    }
...
async function handleFormSubmit(e) {
    ...
    if (!warehouseId || !operationDate) {
        alert("Please fill in all required fields: Warehouse, and Operation Date.");
        hideLoader();
        return;
    }
    ...
    if (reasonInput.value) payload.append("reason", reasonInput.value);
    ...
    try {
        const res = await postRequest(API_ENDPOINTS.Operations.damage, payload, token, {}, true);
